...
"Tariq A.", "Tehseen H.", "Umair A.", "Uzma H.", "Waqar A.", "Wardah H.", "Yasir A.", "Yumna H.", "Zaeem A.", "Zoya H."
        ...
        // --- LIVE WITHDRAWAL TICKER LOGIC (NEW) ---
        function showRandomWithdrawal() {
            ...
            // Randomly select amount (e.g., 500k to 5M coins)
        ...
        function startWithdrawalTicker() {
            // Run instantly and then every 5 seconds
        ...
        async function handleSubmitWithdrawal() {
            ...
            // --- VIP CHECK: Bypass 24-hour frequency check if Limit Increased is active ---
            if (!(isLimitIncreased && !isExpired)) {
                 // ONLY perform 24-hour check if user is NOT a current VIP
                const twentyFourHoursAgo = new Date(Date.now() - TWENTY_FOUR_HOURS_MS);
                const q = query(collection(db, "withdrawals"), where("userId", "==", currentUser.uid), where("timestamp", ">=", twentyFourHoursAgo));
                const recentWithdrawals = await getDocs(q);
                
                if (!recentWithdrawals.empty) {
                    wdErrorMsg.innerText = "Error: You can only submit one withdrawal request per 24 hours (Base Limit). VIP users have unlimited submissions.";
                    wdErrorMsg.style.display='block';
                    return;
                }
            }
        ...
        document.addEventListener('DOMContentLoaded', () => {
             startWithdrawalTicker(); // Start the live ticker
